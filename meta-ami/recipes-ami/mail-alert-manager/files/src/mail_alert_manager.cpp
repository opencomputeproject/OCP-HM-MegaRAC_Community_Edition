...
uint16_t Smtp::sendmail(const std::string& subject, const std::string& msg ){
	...
	if((Smtp::enable == false) || Smtp::host.empty() || 
		(Smtp::port == 0) || Smtp::sender.empty() ||
		Smtp::recipient.empty()){
		return -1;
	}
...
std::tuple<bool,std::string,uint16_t,std::string,std::string> Smtp::getsmtpconfig(){

	std::tuple<bool,std::string,uint16_t,std::string,std::string> smtpcfg;
	smtpcfg = make_tuple(Smtp::enable, Smtp::host, Smtp::port, Smtp::sender, Smtp::recipient);
	return smtpcfg;
}

uint16_t Smtp::setsmtpconfig(const bool enable, const std::string& host, const uint16_t& port,
...
int Smtp::initializeSmtpcfg()
{
	...
	Json smtpConfig = data["Config"];
        ...
        Smtp::recipient = smtpConfig["Recipient"];
