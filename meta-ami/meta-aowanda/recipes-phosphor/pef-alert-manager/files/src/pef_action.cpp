...
static bool getPowerStatus()
{
        ...
        try
        {
                auto method = conn->new_method_call(pwrService,pwrStateObjPath,PROP_INTF, METHOD_GET);
        ...
        {
                phosphor::logging::log<phosphor::logging::level::ERR>("Failed to get PEFControl Value",
...
{
	auto method = conn->new_method_call(pwrService, pwrCtlObjPath,
...
{
        //sample event1
        if((eveMsgData->sensorNum == 0x30) && (eveMsgData->sensorType == 0x01) && ((eveMsgData->eventType & 0x7f) == 0x01)
                && (eveMsgData->eventData[0] == 0x09) && (eveMsgData->eventData[1] == 0xff) && (eveMsgData->eventData[2] == 0xff))
        {
                return true;
        }//sample event2
        else if((eveMsgData->sensorNum == 0x60) && (eveMsgData->sensorType == 0x02) && ((eveMsgData->eventType & 0x7f) == 0x01)
                && (eveMsgData->eventData[0] == 0x02) && (eveMsgData->eventData[1] == 0xff) && (eveMsgData->eventData[2] == 0xff))
        {
                return true;
        }//sample event3
        else if((eveMsgData->sensorNum ==  0x53) && (eveMsgData->sensorType == 0x0c) && ((eveMsgData->eventType & 0x7f) == 0x6f)
                && (eveMsgData->eventData[0] == 0x00) && (eveMsgData->eventData[1] == 0xff) && (eveMsgData->eventData[2] == 0xff))
        {
                return true;
        }
...
static uint16_t sendSmtpAlert(struct EventMsgData *eveMsg,uint8_t eveLog)
{
	...
	if(eveType == static_cast<uint8_t>(EventTypeCode::threshold))
        ...
        {
                auto offset = SENSOR_SPECIFIC_EVENT_TABLE.find(eveMsg->sensorType)->second;
                eventDataMsg = offset.find(evnDat)->second;
        }
        else
        {
	...
	try
        {
		Value variant;
                auto method = conn->new_method_call(networkService,networkObjPath,PROP_INTF, METHOD_GET);
	...
	uint16_t mailstatus = 0;
        try
        {
		auto sendAlert = conn->new_method_call(mailService,mailObjPath,mailIface,sendMailMethod);
...
std::vector<std::string> checkAlertPoicyTbl(int AlertPolicyNo)
{
	...
	for(int index = 1; index <= ALERT_POLICY_SET ; index++)
	{
		AlertPolicyTbl AlertPlyTbl;
                AlertPlyTbl = {};
		std::string AlertPlyObj = alertPolicyTableObj + std::to_string(AlertPolicyNo);
		try
                {
                 	PropertyMap alertPolicyValues;
                        auto method = conn->new_method_call(pefBus,AlertPlyObj.c_str(),PROP_INTF, METHOD_GET_ALL);
  			...
  			AlertPlyTbl.AlertNum = std::get<uint8_t>(alertPolicyValues.at("AlertNum"));
  			//AlertPlyTbl.ChannelDestSel = std::get<uint8_t>(alertPolicyValues.at("ChannelDestSel"));
  			//AlertPlyTbl.AlertStingkey = std::get<uint8_t>(alertPolicyValues.at("AlertStingkey"));
...
static void performPefAction(std::vector<std::string>& matEveFltEntries,struct EventMsgData *eveMsg)
{
	for (int index = 0;index < matEveFltEntries.size(); index++)
	{
		...
		try
                {
                        PropertyMap values;
                	auto method = conn->new_method_call(pefBus,matEveFltEntries[index].c_str(),PROP_INTF, METHOD_GET_ALL);
			...
			if (reply.is_method_error())
    			...
    			reply.read(values);
			eveFltTblEntry.EvtFilterAction = std::get<uint8_t>(values.at("EvtFilterAction"));
			eveFltTblEntry.AlertPolicyNum = std::get<uint8_t>(values.at("AlertPolicyNum"));
			eveFltTblEntry.EventSeverity = std::get<uint8_t>(values.at("EventSeverity"));
		}
		catch (sdbusplus::exception_t& e)
                {
                        phosphor::logging::log<phosphor::logging::level::ERR>(
		...
		try
                {
                        ...
                        auto method = conn->new_method_call(pefBus,pefObj,PROP_INTF, METHOD_GET_ALL);
			...
			pefcfgInfo.PEFControl = std::get<uint8_t>(pefCfgValues.at("PEFControl"));
                        pefcfgInfo.PEFActionGblControl = std::get<uint8_t>(pefCfgValues.at("PEFActionGblControl"));
                        pefcfgInfo.PEFStartupDly = std::get<uint8_t>(pefCfgValues.at("PEFStartupDly"));
                        pefcfgInfo.PEFAlertStartupDly = std::get<uint8_t>(pefCfgValues.at("PEFAlertStartupDly"));
                }
                catch (sdbusplus::exception_t& e)
                {
                        phosphor::logging::log<phosphor::logging::level::ERR>(
		...
		if(((eveFltTblEntry.EvtFilterAction & POWER_OFF_ACTION) == POWER_OFF_ACTION) ||
			((eveFltTblEntry.EvtFilterAction & POWER_CYCLE_ACTION) == POWER_CYCLE_ACTION) ||
			((eveFltTblEntry.EvtFilterAction & RESET_ACTION) == RESET_ACTION))
		{
			if(((eveFltTblEntry.EvtFilterAction & POWER_OFF_ACTION) == POWER_OFF_ACTION)
					&& ((pefcfgInfo.PEFActionGblControl & POWER_OFF_ACTION) == POWER_OFF_ACTION))
			{
				int rc = initiateChassisStateTransition(pwrCtlOff);
				if(rc < 0)
					std::cerr << "Failed to do power action\n";
			}
			else if((((eveFltTblEntry.EvtFilterAction & POWER_CYCLE_ACTION) == POWER_CYCLE_ACTION) && 
				((pefcfgInfo.PEFActionGblControl & POWER_CYCLE_ACTION) == POWER_CYCLE_ACTION)) ||
                        	(((eveFltTblEntry.EvtFilterAction & RESET_ACTION) == RESET_ACTION) && 
				((pefcfgInfo.PEFActionGblControl & RESET_ACTION) == RESET_ACTION)))
			{
				...
				((pefcfgInfo.PEFActionGblControl & ALERT_ACTION) == ALERT_ACTION))
		{
			...
			{
				for(int AlertEntry = 0; AlertEntry < Alertpolicy.size();AlertEntry++ )
				{
					AlertPolicyTbl AlertPlyTbl;
					AlertPlyTbl = {};
					try
                			{
                        			PropertyMap alertPolicyValues;
                				auto method = conn->new_method_call(pefBus,Alertpolicy[AlertEntry].c_str(),
					...
					if(0 != (AlertPlyTbl.AlertNum & 0x08))
					{
						...
						alertStatus = sendSmtpAlert(eveMsg,pefcfgInfo.PEFControl);
						if(alertStatus == 0)
						{
							phosphor::logging::log<phosphor::logging::level::INFO>("Alert Send Sucessfully!!!");
							try
        						{
                						auto method = conn->new_method_call(pefBus, pefObj,
                        						        "org.freedesktop.DBus.Properties", "Set");
                						method.append(pefConfInfoIntf,"LastBMCProcessedEventID");
								method.append(std::variant<uint16_t>(eveMsg->recordId));
                						auto reply = conn->call(method);
        						}
...
static uint8_t pefEveDataMatch(uint8_t value,uint8_t andMask,uint8_t cmp1,uint8_t cmp2)
{
	uint8_t match;
...
static void eventFilteringProcess(struct EventMsgData *eventMsg)
{
	...
	for (int index = 1; index < MAX_EVT_FILTER_ENTRIES; index++)
	{
		...
		EvtFilterTblEntry eveFltTblEntry;
		eveFltTblEntry = {};
		std::string eveFltEntryObj = eventFilterTableObj + std::to_string(index);
		try
        	{
                	//PropertyMap values = getAllDbusProperties(bus,pefBus,eveFltEntryObj,eventFilterTableIntf);
                	PropertyMap values;
                	auto method = conn->new_method_call(pefBus,eveFltEntryObj.c_str(),PROP_INTF, METHOD_GET_ALL);
			method.append(eventFilterTableIntf);
			auto reply = conn->call(method);
			if (reply.is_method_error())
    			{
    			...
    			reply.read(values);
                	eveFltTblEntry.FilterConfig = std::get<uint8_t>(values.at("FilterConfig"));
			eveFltTblEntry.GenIDByte1 = std::get<uint8_t>(values.at("GenIDByte1"));
			eveFltTblEntry.GenIDByte2 = std::get<uint8_t>(values.at("GenIDByte2"));
			eveFltTblEntry.SensorType = std::get<uint8_t>(values.at("SensorType"));
			eveFltTblEntry.SensorNum = std::get<uint8_t>(values.at("SensorNum"));
			eveFltTblEntry.EventTrigger = std::get<uint8_t>(values.at("EventTrigger"));
			eveFltTblEntry.EventData1ANDMask = std::get<uint8_t>(values.at("EventData1ANDMask"));
			eveFltTblEntry.EventData1Cmp1 = std::get<uint8_t>(values.at("EventData1Cmp1"));
			eveFltTblEntry.EventData1Cmp2 = std::get<uint8_t>(values.at("EventData1Cmp2"));
			eveFltTblEntry.EventData2ANDMask = std::get<uint8_t>(values.at("EventData2ANDMask"));
			eveFltTblEntry.EventData2Cmp1 = std::get<uint8_t>(values.at("EventData2Cmp1"));
			eveFltTblEntry.EventData2Cmp2 = std::get<uint8_t>(values.at("EventData2Cmp2"));
			eveFltTblEntry.EventData3ANDMask = std::get<uint8_t>(values.at("EventData3ANDMask"));
			eveFltTblEntry.EventData3Cmp1 = std::get<uint8_t>(values.at("EventData3Cmp1"));
			eveFltTblEntry.EventData3Cmp2 =	std::get<uint8_t>(values.at("EventData3Cmp2"));
			eveFltTblEntry.EventData1OffsetMask = std::get<uint16_t>(values.at("EventData1OffsetMask"));
        	}
        	catch (sdbusplus::exception_t& e)
        	{
                	phosphor::logging::log<phosphor::logging::level::ERR>(
		...
		if (((eveFltTblEntry.GenIDByte1 != 0xFF) && (eveFltTblEntry.GenIDByte1 != eventMsg->generatorId1)) ||
            	   ((eveFltTblEntry.GenIDByte2 != 0xFF) && (eveFltTblEntry.GenIDByte2 != eventMsg->generatorId2)) ||
            	   (0 == eventMsg->sensorType) ||
                   ((eveFltTblEntry.SensorType != 0xFF) && (eveFltTblEntry.SensorType != eventMsg->sensorType))||
                   ((eveFltTblEntry.SensorNum != 0xFF)  && (eveFltTblEntry.SensorNum != eventMsg->sensorNum)) ||
                   ((eveFltTblEntry.EventTrigger != 0xFF)&& (eveFltTblEntry.EventTrigger != eventMsg->eventType)))
		{
			continue;
		}
		//check eventData1
		if (0 == pefEveDataMatch(eventMsg->eventData[0],eveFltTblEntry.EventData1ANDMask,
					eveFltTblEntry.EventData1Cmp1,eveFltTblEntry.EventData1Cmp2))
		{
			continue;
		}
		//check eventData2
		if (0 == pefEveDataMatch(eventMsg->eventData[1],eveFltTblEntry.EventData2ANDMask,
					eveFltTblEntry.EventData2Cmp1,eveFltTblEntry.EventData2Cmp2))
                {
                        continue;
                }
		//check eventData3
		if (0 == pefEveDataMatch(eventMsg->eventData[2],eveFltTblEntry.EventData3ANDMask,
					eveFltTblEntry.EventData3Cmp1,eveFltTblEntry.EventData3Cmp2))
                {
                        continue;
                }
...
static void pefTask(const uint16_t &recId,const uint8_t &senType, const uint8_t &senNum, const uint8_t &eveType,
		const uint8_t &eveData1,const uint8_t &eveData2,const uint8_t &eveData3, const uint16_t& genId)
{
	EventMsgData eveMsg = {};
	...
	try
	{
		auto method = conn->new_method_call(pefBus,pefObj,PROP_INTF, METHOD_GET);
	...
	uint8_t pefPostponeTimer = 0;
        ...
        try
        {
                auto method = conn->new_method_call(pefBus,pefPostponeTmrObj,PROP_INTF, METHOD_GET);
	...
	if((pefPostponeTimer == 0xFE) || ((pefPostponeTimer != 0x00) && (pefPostponeTimer =! 0xFF)))
	{
		phosphor::logging::log<phosphor::logging::level::ERR>(
